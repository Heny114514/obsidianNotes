...
## 断言
**断言（Assertion）** 是数据库级的完整性约束。它不隶属于某一张表，可以引用多个表、多个元组以及聚集查询，用于表达 `CHECK`、主键和外键难以直接表达的全局约束。
...
- `CHECK` 中的条件在任意合法数据库状态下都必须为真；
- 数据库执行可能影响该条件的 `INSERT`、`UPDATE` 或 `DELETE` 时，会检查断言；若条件为假，则拒绝该操作（或回滚该事务）；
- 通常以 `NOT EXISTS` 表达“不允许存在违反约束的元组”。

### 示例 1：跨表成绩不能超过课程满分

设 `Course(Cno, Cname, FullMark)` 记录课程满分，`SC(Sno, Cno, Grade)` 记录选课成绩。要求任一成绩不得超过对应课程的满分：

```sql
CREATE ASSERTION grade_not_over_full_mark
CHECK (
    NOT EXISTS (
        SELECT 1
        FROM SC AS sc
        JOIN Course AS c ON sc.Cno = c.Cno
        WHERE sc.Grade > c.FullMark
    )
);
```

若插入或更新一条使 `sc.Grade > c.FullMark` 的记录，断言条件变为假，该操作不能提交。

### 示例 2：限制全表元组数量

要求 `Student` 表中的学生总数不超过 5 000：

```sql
CREATE ASSERTION student_count_limit
CHECK (
    (SELECT COUNT(*) FROM Student) <= 5000
);
```
...
| 机制 | 约束范围 | 典型用途 |
|---|---|---|
| 列/表级 `CHECK` | 当前表的当前行或表级条件 | 分数范围、取值集合等简单约束 |
| `CREATE ASSERTION` | 整个数据库，可跨表、可使用子查询/聚集 | 跨表比较、总量限制、全局业务规则 |
| 触发器 | 由特定事件触发，可执行过程化动作 | 断言不支持时的替代方案、审计与自动维护 |

> `CREATE ASSERTION` 是 SQL 标准语法，但部分数据库产品未实现。实际系统不支持时，通常用触发器或应用层校验实现同类业务规则；无论采用何种方式，都应在数据库层保留关键约束的兜底校验。
...
3. **左部最简化**：对每个 $X \rightarrow A$，检查 $X$ 中是否有冗余属性
   - 对 $X$ 的每个属性 $B$，令 $Z = X \setminus \{B\}$；若在当前依赖集 $F$ 中计算得 $A \in Z^+$，则 $B$ 冗余，用 $Z$ 代替 $X$

**完整示例**：求 $F = \{AB \rightarrow CD,\; A \rightarrow B,\; C \rightarrow D,\; A \rightarrow D\}$ 的最小依赖集。本例会同时出现**冗余依赖**和**左部冗余属性**。

**Step 1 — 右部分解为单属性**：

| 原依赖 | 分解后 |
|--------|--------|
| $AB \rightarrow CD$ | $AB \rightarrow C$, $AB \rightarrow D$ |
| $A \rightarrow B$ | $A \rightarrow B$ |
| $C \rightarrow D$ | $C \rightarrow D$ |
| $A \rightarrow D$ | $A \rightarrow D$ |

得 $F_1 = \{AB \rightarrow C,\; AB \rightarrow D,\; A \rightarrow B,\; C \rightarrow D,\; A \rightarrow D\}$。

**Step 2 — 去除冗余依赖**：

- 检查 $AB \rightarrow D$：暂时移除它。由 $AB \rightarrow C$ 与 $C \rightarrow D$ 可推出 $AB \rightarrow D$，即在其余依赖集中 $(AB)^+$ 含有 $D$，故该依赖**冗余**，删除 ✅
- 检查 $A \rightarrow D$：暂时移除它。由 $A \rightarrow B$，再结合 $AB \rightarrow C$ 和 $C \rightarrow D$，可推出 $A \rightarrow D$；即其余依赖集中 $A^+$ 含有 $D$，故该依赖也**冗余**，删除 ✅
- $A \rightarrow B$、$AB \rightarrow C$、$C \rightarrow D$ 均不能由其余依赖推出，予以保留。

得 $F_2 = \{AB \rightarrow C,\; A \rightarrow B,\; C \rightarrow D\}$。

**Step 3 — 左部最简化**：

- 检查 $AB \rightarrow C$ 的左部。去掉 $B$ 后计算 $A^+$：先由 $A \rightarrow B$ 得到 $B$，再由 $AB \rightarrow C$ 得到 $C$，因此 $C \in A^+$。故 $B$ 是左部冗余属性，$AB \rightarrow C$ 可替换为 $A \rightarrow C$ ✅
- 再检查 $A$ 是否冗余：$B^+=\{B\}$，不能推出 $C$，故 $A$ 不冗余。
- $A \rightarrow B$ 和 $C \rightarrow D$ 的左部均只有一个属性，无需简化。

故最小依赖集为 $F_m = \{A \rightarrow B,\; A \rightarrow C,\; C \rightarrow D\}$。
